...
class Client {
	...
	async submitTask(data: TaskData, queueName: string): Promise<string> {
		try {
	...
	async getResult(taskId: string, isWait = true): Promise<TaskResult | null> {
	...
	async getFile(fileUrl: string, filePath: string): Promise<File> {
		try {
...
client
	.getFile("file_url", "path/to/download/file.txt")
